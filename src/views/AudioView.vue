        ...
        <div class="w-full min-h-full flex items-center justify-center ">
            ...
            <ul class="border-t border-white">
                <li v-for="page in state.accessInfoTranslated">
                    <button class="w-full text-left text-lg xl:text-2xl py-3 border-b border-white hover:text-yellow" 
                        aria-label="{{ page.title }}" @click="loadfile(page.audio_file)">
                        {{ page.title }}
                        <span v-if="page.audio_file_length"> - {{page.audio_file_length}}</span>
                    </button>
